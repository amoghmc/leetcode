#!/home/linuxbrew/.linuxbrew/bin/python3
"""
Suppose an array of length n sorted in ascending order
is rotated between 1 and n times.

For example, the array nums = [0,1,2,4,5,6,7] might become:

[4,5,6,7,0,1,2] if it was rotated 4 times.
[0,1,2,4,5,6,7] if it was rotated 7 times.
Notice that rotating an array [a[0], a[1], a[2], ..., a[n-1]] 1 time
 results in the array [a[n-1], a[0], a[1], a[2], ..., a[n-2]].

Given the sorted rotated array nums of unique elements,
 return the minimum element of this array.

You must write an algorithm that runs in O(log n) time.



Example 1:
	Input: nums = [3,4,5,1,2]
	Output: 1
	Explanation: The original array was [1,2,3,4,5] rotated 3 times.

Example 2:
	Input: nums = [4,5,6,7,0,1,2]
	Output: 0
	Explanation: The original array was [0,1,2,4,5,6,7] and it was rotated 4 times.

Example 3:
	Input: nums = [11,13,15,17]
	Output: 11
	Explanation: The original array was [11,13,15,17] and it was rotated 4 times.

Constraints:
	n == nums.length
	1 <= n <= 5000
	-5000 <= nums[i] <= 5000
	All the integers of nums are unique.
	nums is sorted and rotated between 1 and n times.

https://leetcode.com/problems/find-minimum-in-rotated-sorted-array/
"""
import unittest
from typing import List


class Solution:
	def findMin(self, nums: List[int]) -> int:
		left_ptr, right_ptr = 0, len(nums) - 1

		target = float('inf')
		while left_ptr <= right_ptr:
			# if array is sorted without rotation
			if nums[left_ptr] < nums[right_ptr]:
				target = min(target, nums[left_ptr])

			mid = (left_ptr + right_ptr) // 2
			target = min(target, nums[mid])

			# left sorted portion
			if nums[left_ptr] <= nums[mid]:
				left_ptr = mid + 1

			# right sorted portion
			else:
				right_ptr = mid - 1

		return target


class TestSolution(unittest.TestCase):
	def tests(self):
		sol_class = Solution()
		my_functions = [sol_class.findMin]
		for my_function in my_functions:
			self.assertEqual(my_function([3, 4, 5, 1, 2]), 1)
			self.assertEqual(my_function([11, 13, 15, 17]), 11)
			self.assertEqual(my_function([4, 5, 6, 7, 0, 1, 2]), 0)


if __name__ == '__main__':
	unittest.main()
